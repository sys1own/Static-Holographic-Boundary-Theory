...
_tier_1_topological_coordinates = _constants.TIER_1_TOPOLOGICAL_COORDINATES
_tier_3_derived_residues = _constants.TIER_3_DERIVED_RESIDUES

G_SM = _tier_1_topological_coordinates["G_SM"]
...
N_holo = _tier_3_derived_residues["HOLOGRAPHIC_BITS"]
...
BENCHMARK_C_DARK_RESIDUE_FRACTION = _tier_3_derived_residues["BENCHMARK_C_DARK_RESIDUE_FRACTION"]
BENCHMARK_C_DARK_RESIDUE = _tier_3_derived_residues["BENCHMARK_C_DARK_RESIDUE"]
BENCHMARK_REFERENCE_COSET_CENTRAL_CHARGE_FRACTION = _tier_3_derived_residues[
    "BENCHMARK_REFERENCE_COSET_CENTRAL_CHARGE_FRACTION"
]
BENCHMARK_REFERENCE_COSET_CENTRAL_CHARGE = _tier_3_derived_residues["BENCHMARK_REFERENCE_COSET_CENTRAL_CHARGE"]
...
CKM_PHASE_TILT_PARAMETER = _tier_3_derived_residues["R_GUT"]
...
def _pull_table_to_tex(self: PullTable) -> str:
    ...
    kappa_sym = r"\\kappa_{D_5}"
    lambda_sym = r"\\Lambda_{\\rm obs}"
    calibration_summary = (
        f"Geometric anchor: {getattr(self, 'calibration_input_symbol', kappa_sym)}"
        f" = {getattr(self, 'calibration_input_value', math.nan)}"
    )
    anchor_summary = (
        f"Cosmology anchor: {getattr(self, 'cosmology_anchor_symbol', lambda_sym)}"
...
def _latex_scientific_notation(value: float, *, digits: int = 5) -> str:
    ...
    if math.isclose(numeric_value, 0.0, rel_tol=0.0, abs_tol=0.0):
        return "0"
...
def _sovereign_default_physics_constants_text() -> str:
    planck_mass_ev = (HBAR_EV_SECONDS * LIGHT_SPEED_M_PER_S) / PLANCK_LENGTH_M
    newton_coordinate_ev_minus2 = 1.0 / (planck_mass_ev * planck_mass_ev)
    c_dark_completion_fraction = Fraction(1197103, 362670)
    lines = (
        "% Sovereign Default generated by pub/tn.py pre-flight resource check.",
        "% Provides the benchmark gravity/flavor cross-reference macros when manuscript artifacts are absent.",
        rf"\newcommand{{\simplexPrefactor}}{{{KAPPA_D5:.5f}}}",
        rf"\newcommand{{\mZeroBenchmarkMeV}}{{{1.0e3 * BENCHMARK_LOW_SCALE_LIGHTEST_MASS_EV:.2f}}}",
        rf"\newcommand{{\cDarkCompletionExact}}{{\frac{{{c_dark_completion_fraction.numerator}}}{{{c_dark_completion_fraction.denominator}}}}}",
        rf"\newcommand{{\benchmarkNewtonGEvMinusTwo}}{{{_latex_scientific_notation(newton_coordinate_ev_minus2)}}}",
        rf"\newcommand{{\benchmarkPlanckMassEv}}{{{_latex_scientific_notation(planck_mass_ev)}}}",
        rf"\newcommand{{\benchmarkLambdaMetersInverseSquared}}{{{_latex_scientific_notation(PLANCK2018_LAMBDA_SI_M2)}}}",
        rf"\newcommand{{\lambdaHoloMetersInverseSquared}}{{{_latex_scientific_notation(PLANCK2018_LAMBDA_SI_M2)}}}",
    )
    return "\n".join(lines) + "\n"


def _sovereign_default_tolerance_table_text() -> str:
    return "\n".join(
        (
            "% Sovereign Default supplementary_tolerance_table.tex generated by pub/tn.py.",
            "% Baseline Radau vs RK45 tolerance benchmarks retained for pre-flight sector audits.",
            r"\begin{tabular}{ccccc}",
            r"\toprule",
            r"rtol & atol & $\chi^2_{\rm pred}$ & $|\Delta\chi^2|$ vs. $10^{-12}$ & max $\sigma$-shift \\",
            r"\midrule",
            r"$10^{-8}$ & $10^{-10}$ & 3.386480991 & 1.226e-09 & 9.513e-10 \\",
            r"$10^{-10}$ & $10^{-12}$ & 3.386480990 & 6.988e-12 & 5.373e-12 \\",
            r"$10^{-12}$ & $10^{-14}$ & 3.386480990 & 0.000e+00 & 0.000e+00 \\",
            r"\bottomrule",
            r"\end{tabular}",
        )
    ) + "\n"
...
def _ensure_audit_resources() -> tuple[Path, ...]:
    resource_dir = _audit_resource_dir()
    resource_dir.mkdir(parents=True, exist_ok=True)
    resources = (
        (resource_dir / "physics_constants.tex", _sovereign_default_physics_constants_text()),
        (resource_dir / "supplementary_tolerance_table.tex", _sovereign_default_tolerance_table_text()),
    )
    written_paths: list[Path] = []
    for resource_path, default_text in resources:
        if resource_path.exists():
            continue
        resource_path.write_text(default_text, encoding="utf-8")
        LOGGER.info("[AUDIT PREFLIGHT]: Wrote sovereign default %s", _display_path(resource_path))
        written_paths.append(resource_path)
