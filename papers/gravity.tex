    ...
    \IfFileExists{papers/#1}{\input{papers/#1}}{%
      \IfFileExists{../papers/#1}{\input{../papers/#1}}{%
        \IfFileExists{pub/#1}{\input{pub/#1}}{%
          \PackageWarning{gravity}{Input file `#1' not found; continuing without it}%
        }%
      }%
    }%
  }%
...
\IfFileExists{tn.aux}{%
  ...
  \IfFileExists{papers/tn.aux}{\usepackage{xr}\externaldocument[tn-]{papers/tn}}{%
    \IfFileExists{../papers/tn.aux}{\usepackage{xr}\externaldocument[tn-]{../papers/tn}}{%
...
\emph{Holographic Surface Tension} required to bind the completed
$SO(10)_{312}$ partition function to the finite one-copy register
$N_{\rm sat}$ and to keep the anomaly-free $(26,8,312)$ cell inside the
fixed-parent local moat singled out by $\Delta_{\rm fr}=0$. The neighboring
cells $(25,8,312)$ and $(27,8,312)$ are not nearby vacua of the same branch:
they already sit outside that moat because their vacuum modular-$T$ powers are
fractional and therefore reopen the framing defect. On the retained de Sitter
branch this closure requirement is equivalently the finite-capacity statement
\begin{equation}
\label{eq:gravity-surface-tension-necessity}
 \Lambda_{\rm holo}>0
 ...
 N_{\rm sat}<\infty.
\end{equation}
If one were to send $\Lambda_{\rm holo}\to0^+$, then
...
spinorial-retention factor entering the geometric residue by
\begin{equation}
\label{eq:gravity-spinorial-retention}
 \eta_{\rm spin}
 \equiv
 ...
 0.91844,
 \qquad
 \beta\equiv \frac12\ln\!\left(\frac{\sqrt{14}}{\sin(\pi/28)}\right),
\end{equation}
so the first entry $\kappa_{D_5}=\simplexPrefactor$ remains the fixed
$D_5$ hyperarea residue dressed by the parent spinorial bookkeeping rather
than a free matching dial. Define the
...
\end{equation}
with $\mathcal Q_{\rm iso}\neq 0$ the branch-fixed isotropic anomaly density
determined by the parent, visible, and completion central charges. The branch
...
\begin{equation}
 ...
 \Delta_{\rm fr}=0.
\end{equation}
Bulk diffeomorphism invariance is therefore not an independent postulate in
this benchmark appendix: it is the branch's \emph{Noether bridge}, namely the
transport of the boundary framing Ward identity into the bulk, with the
contracted Bianchi identity as its covariant image. In this narrow
appendix-level sense, ``Noether bridge'' refers only to conservation-law
transport inside the static SHBT bookkeeping; it is not a separate microscopic
derivation beyond the branch-fixed closure assumptions.
...
Section~G.3 gives the expanded contradiction proof in the local-moat and
tilt-consistency language, but the lock itself is already forced by the
Topological EFE identity.
...
\manualappendixlabel{G.3}{app:topological-moat-consistency}
\manualappendixlabel{G.3}{app:topological-moire-consistency}
\subsection*{G.3: Local-Moat Selection, Topological Moir\'e Tilt, Vacuum Loading, and the Bianchi Lock}

The branch-selection problem and the scalar-tilt problem are related but not
identical. In the discrete visible-level scan, the anomaly-free cell
$(26,8,312)$ is isolated by the fixed-parent local moat: the nearest
neighbors $(25,8,312)$ and $(27,8,312)$ already carry
$\Delta_{\rm fr}=0.24$ and $2/9$, so they are excluded before any
phenomenological ranking is attempted. Once that moat-selected lattice is
projected onto a smooth four-dimensional horizon geometry, the scalar
departure from exact scale invariance appears as the long-wavelength
\emph{Topological Moir\'e Pattern}. Writing the scalar mismatch as
\begin{equation}
...
deficit $\delta_{\rm topo}=1-\kappa_{D_5}$. If the Hubble friction
$3H_{\Lambda}$ did not exactly match the geometric cancellation required by
$\delta_{\rm topo}$, then $\Delta_{\rm lock}\neq0$, so
Eq.~\eqref{eq:gravity-frw-lock-mismatch} would imply
$\nabla^{\mu}T^{\rm vis}_{\mu 0}\neq0$ in contradiction with
Eq.~\eqref{eq:gravity-visible-stress-conservation}. This is the
Bianchi/friction contradiction that fixes the isotropic damping coefficient on
the anomaly-free branch.
