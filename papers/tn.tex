...
\usepackage{xr}
\graphicspath{{results/final/}{../results/final/}{results/}{../results/}{./}{papers/}{../papers/}{pub/}}

\newif\ifartifactfiguresavailable
\artifactfiguresavailablefalse
\IfFileExists{fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{%
  \IfFileExists{results/final/fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{%
    \IfFileExists{../results/final/fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{%
      \IfFileExists{results/fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{%
        \IfFileExists{../results/fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{%
          \IfFileExists{papers/fig1_pmns_fit.png}{\artifactfiguresavailabletrue}{}%
        }%
      }%
    }%
  }%
}
\ifartifactfiguresavailable\else
  \setkeys{Gin}{draft}
\fi
      ...
      \IfFileExists{results/#1}{\input{results/#1}}{%
        \IfFileExists{../results/#1}{\input{../results/#1}}{%
          \IfFileExists{#1}{\input{#1}}{%
            \IfFileExists{papers/#1}{\input{papers/#1}}{%
              \IfFileExists{../papers/#1}{\input{../papers/#1}}{%
                \IfFileExists{pub/#1}{\input{pub/#1}}{%
                  \PackageWarning{tn}{Input file `#1' not found; continuing without it}%
                }%
              }%
            }%
          }%
...
\IfFileExists{supplementary.aux}{\externaldocument[supp-]{supplementary}}{%
  \IfFileExists{papers/supplementary.aux}{\externaldocument[supp-]{papers/supplementary}}{%
    \IfFileExists{../papers/supplementary.aux}{\externaldocument[supp-]{../papers/supplementary}}{%
...
\begin{abstract}
We present the static $SO(10)_{312}$ boundary construction as a first-principles
topological closure problem. The parent/visible data are fixed by exact WZW
branching integrality, Witten framing quantization, and GKO Virasoro
orthogonality; the disclosed $9{,}801$-cell \emph{Local Moat Audit} is retained
only as a numerical confirmation of that analytic selection. Within the minimal
canonical $SO(10)\to SU(3)\times SU(2)$ completion used here, the
\emph{Derived Uniqueness Theorem} selects $(k_{\ell},k_q,K)=(26,8,312)$ as the
unique \emph{Minimal Anomaly-Free Local Survivor}. On that branch the framing
defect vanishes, $\Delta_{\rm fr}=0$, so the Bulk Closure Tensor
$\mathcal E_{\mu\nu}$ also vanishes and the emergent bulk description preserves
diffeomorphism invariance rather than assuming it as an independent postulate.

On the retained anomaly-free branch, modular-$T$ closure is not compatible with
a vanishing vacuum term: the same bookkeeping requires a strictly positive
holographic vacuum loading $\Lambda_{\rm holo}>0$, equivalently a finite
boundary register $N=3\pi/(L_P^2\Lambda_{\rm holo})$. If
$\Lambda_{\rm holo}$ were allowed to vanish, $N$ would decompactify and the
branch-fixed neutrino floor would collapse.
...
triplet $(26,8,312)$, the visible-current neutrality factor $G_{\rm SM}=15$,
the representational admissibility constraint
$\langle \Sigma_{126}\rangle/\langle\phi_{10}\rangle=64/312$, and the
observation-anchored bit budget $N$ inferred from $\Lambda_{\rm obs}$ are
fixed, the construction retains no detached continuous adjustment sector. Any
independent detuning of the integers $(26,8,312)$ reopens the framing anomaly
$\Delta_{\rm fr}\neq0$, takes the construction off the normalizable
finite-capacity branch, and with it reopens the bulk closure defect. The
...
\subsection*{Glossary Box: Boundary-Selection Definitions}

Several recurrent terms are used in a narrow branch-selection sense throughout the paper. For quick reference, Table~\ref{tab:glossary-box} collects the main navigation cues used in the load-bearing argument, while Table~\ref{tab:notation-definitions} serves as the single formal definition block for the core selected-branch symbols.
...
\hline
Derived Uniqueness Theorem / Framing Gap $\Delta_{\rm fr}$ & Branch-selection theorem and operational framing diagnostic; its formal definition and selected-branch value are collected once in Table~\ref{tab:notation-definitions}. \\
Modularity Gap $\Delta_{\rm mod}$ & Residual central-charge mismatch reported only after theorem-level framing filtering; it does not select the branch by itself. \\
...
Selected Branch & The fixed-parent visible branch $(k_{\ell},k_q,K)=(26,8,312)$ carried forward to phenomenology after the anomaly-cancellation filter is applied. \\
...
Residual Audit & The eight-row selected-branch consistency metric $\chi_{\rm pred}^2$ together with the gauge-side consistency pulls; the separate RG Consistency Audit carries the displayed mass-relation rows without entering that tally. \\
\hline
\end{tabularx}
\caption{Glossary box for the boundary-selection terminology and reviewer-navigation cues used in the Introduction and the main discussion.}
...
For reader convenience we collect the fixed selected-branch constants that recur in
...
\hline
Symbol & Definition & Value on selected branch \\
\hline
...
$\Delta_{\rm fr}$ & $\operatorname{dist}\!\left(K/(2k_{\ell}),\mathbb Z\right)$ & $0$ on the selected branch \\
...
\hline
\end{tabularx}
\caption{Notation and definitions for the fixed anomaly-free branch $(26,8,312)$. This table is the single formal definition block for the core selected-branch symbols used below.}
...
\par\smallskip
\textbf{(1) Topological Foundation and Branch Selection.} Apply the Derived Uniqueness Theorem: exact Diophantine closure $K=\operatorname{lcm}(2k_{\ell},3k_q)$, Witten framing quantization, and GKO orthogonality retain only the visible cell $(26,8,312)$.
\par\smallskip
...
\par\smallskip
\textbf{(6) Residual Audit.} Report the eight-row selected-branch consistency metric $\chi_{\rm pred}^2$ together with the gauge-side pulls, while carrying the absolute-mass branch prediction through the separate RG Consistency Audit.
\end{minipage}}

\subsection*{Observational Boundary Conditions and Topological Selection Hypotheses}

Table 0 isolates the structural constraints and observation-anchored inputs that are disclosed rather than derived inside the flavor transport.
...
\begin{equation}
 ...
 \frac{64}{312}.
\end{equation}
We therefore treat $64/312$ as the branch's disclosed \emph{Representational Admissibility Constraint}, equivalently the scalar-interface \emph{Topological Selection Hypothesis}. It is the exact inverse-Clebsch rational value associated with the selected branch and is required by the Scalar Ward Identity for basis invertibility of the charged-fermion map. In the selected-branch bookkeeping it enters as a leading-order scalar-sector singular-value relation, but it is not a value tuned to repair the $b/\tau$ hierarchy or to improve the fit. Even though the Higgs potential itself is not solved in this paper, the ratio is treated as a fixed branch-compatible constraint rather than as a hidden continuous scan variable. The full selected-branch construction is conditioned on the fixed Topological Coordinate triplet, the external observational boundary condition $N$, and the disclosed structural residues.\footnote{Detailed runtime anchors and reproducibility metadata are collected in Appendix~\ref{app:computational}. They document the synchronized implementation but do not enter the branch-selection logic itself.}
...
\hline
Disclosure tier & Branch datum / structural residue & Selection principle & Role in the mapping to observed units \\
\hline
...
Tier 2: External Boundary Conditions & Cosmological bit budget $N$ & Observational Boundary Condition & Observation-anchored boundary input for the selected-branch mass relation $m_\nu\propto N^{-1/4}$ \\
Tier 3: Structural Residues & \shortstack[l]{Current-Algebra\\Neutrality\\$G_{\rm SM}=15$} & visible-current neutrality & Structural visible-current normalization of the gauge proxy, with the SM-singlet excluded from visible transport \\
Tier 3: Topological Selection Hypothesis & \shortstack[l]{Representational\\Admissibility\\$64/312$} & Scalar Ward Identity / basis invertibility & Branch-fixed singular-value admissibility constraint for the charged-fermion map \\
\hline
\end{tabularx}
\caption{Disclosure block for the fixed $(26,8,312)$ branch. Tier~1 lists rigid discrete branch data, Tier~2 lists the observation-anchored cosmological boundary input, and Tier~3 lists the structural residues and topological selection hypotheses used to map the modular kernel to observed units.}
\label{tab:benchmark-input-disclosure}
\end{table}

Table~\ref{tab:benchmark-input-disclosure} is a disclosure block rather than a scan menu: it records the fixed branch labels, the observation-anchored cosmological boundary condition, and the primary flavor-sector structural data used to map the topological kernel to physical units. The generation-weight factor $G_{\rm SM}=15$ is exported explicitly for audit transparency, but its value is not an auxiliary floating normalization: it is the Current-Algebra Neutrality factor of the one-generation visible current algebra of the parent $SO(10)$ spinor, namely $16_F-1=15$ once the SM-singlet direction is removed from the visible current-current correlator. By contrast, the VEV-alignment ratio $64/312$ is treated as the scalar-interface Topological Selection Hypothesis, equivalently a disclosed flavor-side Representational Admissibility Constraint. It is the exact inverse of the $\mathbf{126}_H$ Clebsch--Gordan coefficient associated with the chosen $SU(2)\times SU(3)$ branching,
\begin{equation}
 \frac{\langle\Sigma_{126}\rangle}{\langle\phi_{10}\rangle}
 =
 \frac{h^{\vee}_{SU(2)}}{h^{\vee}_{SU(3)}}\frac{k_q}{k_{\ell}}
...
The selected branch is fixed by three analytic constraints, not by a minimal-embedding assumption. First, the affine WZW branching rules require every embedded visible current block to satisfy
\begin{equation}
 K=x_i\,I_i\,k_i,
\end{equation}
...
\begin{equation}
 ...
 I_Q=\frac{K}{3k_q}.
\end{equation}
Hence both descendant indices must be integers. For the selected visible pair $(k_{\ell},k_q)=(26,8)$ one obtains
\begin{equation}
 K=\operatorname{lcm}(2k_{\ell},3k_q)=\operatorname{lcm}(52,24)=312,
 \qquad
 I_L=6,
...
\paragraph{Exact kernel construction.}
The construction is algorithmic rather than empirical. The visible charges are fixed by the right-edge charge embedding $q_{\alpha}=(k_{\ell}-4,k_{\ell}-3,k_{\ell})$, the quark block by the low-weight set $\Lambda_a\in\{(0,0),(1,0),(0,1)\}$, and the physical unitaries by the polar factor of the restricted modular block multiplied by the framing phases. Because the parent level and descendant indices are already fixed by exact Diophantine closure, the matrices carried into flavor transport are direct images of exact modular data rather than fitted overlap templates.
...
\begin{equation}
 ...
 \qquad f=\ell,q.
\end{equation}
The topological boundary-to-bulk map is then
\begin{align}
...
\begin{equation}
 ...
 = 4.875.
\end{equation}
This overpredicts the observed down-type quark / charged-lepton ratio, but that mismatch does not reopen a continuous Yukawa direction. Instead it identifies the branch-fixed scalar-sector admissibility condition. The selected branch therefore imposes the scalar-interface \emph{Topological Selection Hypothesis}, equivalently the \emph{Representational Admissibility Constraint}
\begin{equation}
 \frac{\langle\Sigma_{126}\rangle}{\langle\phi_{10}\rangle}
 \equiv
 (C_{126}^{(12)})^{-1}
...
\begin{equation}
 r_*=\frac{64}{312}.
\end{equation}
The selected-branch value $r_*=64/312$ is thus the exact inverse of the $\mathbf{126}_H$ Clebsch--Gordan coefficient for the selected branch~\cite{georgi1979,babu1993}. It is used as a topological selection hypothesis for the electroweak interface, not as a free matching prescription or as a theorem that the topological kernel alone fixes the charged-fermion singular values. While the topological kernel provides rigid mixing eigenvectors, the ratio $64/312$ is the unique scalar-interface datum compatible with the Scalar Ward Identity on the anomaly-free branch and suppresses the bare model's over-prediction of the quark/lepton hierarchy without reopening a continuous parameter direction. The SVD Rigidity audit of Fig.~\ref{fig:vev-stability-main}, together with Supplementary Table~\ref{supp-tab:s3-vev-protection} and Supplementary Fig.~\ref{supp-fig:s3-vev-stability}, shows that even $O(10\%)$ deformations of this disclosed setting leave the PMNS/CKM angles far inside their quoted $1\sigma$ windows. The scalar sector therefore renormalizes singular values without relaxing the topological eigenvectors, securing the flavor predictions against scalar-potential uncertainty.

\section{The Unification of Spacetime and Flavor}\label{sec:topological-baryogenesis}

The anomaly-free selected branch does not solve flavor first and append gravity or
...
\paragraph{Isometry proof from branch closure.}
Appendix~\ref{app:gravity} makes the gravity statement explicit by defining the Bulk Closure Tensor
\begin{equation}
 \mathcal E_{\mu\nu}
 \equiv
 \frac{c_{\rm dark}M_P^2}{12}\bigl(G_{\mu\nu}+\Lambda_{\rm holo}g_{\mu\nu}\bigr)-T^{\rm vis}_{\mu\nu}
 =
 \mathcal Q_{\rm iso}\,\Delta_{\rm fr}\,g_{\mu\nu}.
\end{equation}
Because the Derived Uniqueness Theorem retains only the singleton $(26,8,312)$ with $\Delta_{\rm fr}=0$, the selected branch obeys $\mathcal E_{\mu\nu}=0$. The contracted Bianchi identity then gives $\nabla^{\mu}\mathcal E_{\mu\nu}=0$, hence $\nabla^{\mu}T^{\rm vis}_{\mu\nu}=0$ on the retained branch. Bulk diffeomorphism invariance is therefore not an auxiliary assumption added after the flavor construction; it is the covariant image of the same boundary modular-$T$ closure that selects the branch in the first place.

\paragraph{Sakharov constraints at the topological boundary.}
On the selected branch, Sakharov's three conditions are re-expressed as discrete
...
312\mathbb Z,
\end{equation}
and the selected-branch parent level is fixed by the minimality requirement: it is the least common multiple of the branch integers $2k_{\ell}=52$ and $3k_q=24$,
\begin{equation}
 \boxed{K=\operatorname{lcm}(2k_{\ell},3k_q)=312.}
\end{equation}
This is the minimal integer for which both visible branches can be embedded into a common parent without fractional descendants, and in the present paper it is treated as the UV-scale vacuum label of the selected branch. It gives
\begin{equation}
...
\section{Selected-Branch Evaluation}
...
\subsection{Primary Selected-Branch Consistency Metric}
...
\end{equation}
By the \emph{Minimal One-Copy Dictionary} we mean the selected-branch support map in which the three light neutrino mass eigenstates are assigned to the visible genus ladder $g\in\{0,1,2\}$ with at most one state per genus channel and with no duplicated support slots or auxiliary multi-copy sectors. On the anomaly-free branch this is not merely a notational convenience: for the interaction pattern $16_F16_F10_H\oplus16_F16_F\overline{\mathbf{126}}_H$, each visible genus slot may carry only one boundary precursor if the interaction map is to remain rank-preserving. The load-bearing requirement is therefore that the transition map $\mathcal T$ preserve rank on this three-state subspace. All statements about NH and IH in this section refer only to that minimal dictionary.
...
\end{equation}
The same diagnostics also justify the theoretical uncertainties reported in the flavor-side pull tables. Appendix~\ref{app:rg} collects the projected two-loop curvature coefficients along the benchmark trajectory, and the companion supplementary two-loop curvature diagnostics show that the observable-level dressings remain numerically tiny: the largest displayed angular correction is only $\sim10^{-2}\sigma$, while the mass-side two-loop shift above is only $3.7\times10^{-3}\,\mathrm{meV}$. Theoretical uncertainties are defined by the \textbf{Quantified Two-Loop Residuals} derived from the branch-fixed transport audit. The benchmark mass rows are instead reported as fixed outputs of the finite-capacity theorem once $\Lambda_{\rm obs}$ fixes the external bit budget as an \emph{Observational Boundary Condition}, with robustness sweeps retained only as auxiliary diagnostics. The conservative $5\%$ allowance adopted for the flavor pulls is therefore a reviewer-facing diagnostic ceiling rather than a fit window or an independently floated theory band. Its role is to show that the benchmark conclusions remain stable even after granting a buffer far larger than the computed higher-loop residues; the central predictions still remain locked by the branch integers $(26,8,312)$ rather than drifting toward the data. As demonstrated in Table~\ref{supp-tab:s4-two-loop}, the projected higher-loop corrections are $O(10^{-3})$ relative to this ceiling.
...
The manuscript packages the rigid/matched split as a \emph{Topological Selection Hypothesis}. In this reading, the effective four-dimensional Yukawa tensor inherits its support pattern from the RCFT three-point functions of the anomaly-free boundary block: the allowed couplings are those selected by the Verlinde fusion algebra, the restricted modular $S$-matrix supplies the polar unitary kernel, and the framing phases carry the CP-sensitive holonomy. In short, the prescription ``mixing $=$ modular $S$-matrix $+$ framing phases'' is applied once on the fixed branch and is used to account simultaneously for all eight displayed flavor observables $(\theta_{12},\theta_{13},\theta_{23},\delta_{CP},|V_{us}|,|V_{cb}|,|V_{ub}|,\gamma)$. The one-copy dictionary is correspondingly not treated as a disposable placeholder: on the selected branch it is the minimal anomaly-free support assignment compatible with the displayed $SO(10)$ spinor-Higgs couplings $16_F16_F10_H$ and $16_F16_F\overline{\mathbf{126}}_H$. The more granular character-index / generation dictionary is therefore deferred to Supplementary Sec.~\ref{supp-supp-sec:one-copy-character-dictionary}, while the main text retains only the high-level selection principle and its stability audit. The factorization is not claimed as a derived theorem of the full Higgs potential; it is the topological selection hypothesis used to organize the rigid/matched split, and it is stress-tested by the SVD Rigidity audits in Fig.~\ref{fig:vev-stability-main}, Supplementary Table~\ref{supp-tab:s3-vev-protection}, and Supplementary Fig.~\ref{supp-fig:s3-vev-stability}, where benchmark-centered scalar-alignment deformations leave the PMNS/CKM angles unchanged at the quoted precision and thereby secure the flavor predictions against scalar-potential uncertainty.
...
Appendix~\ref{app:rg} records the solar-angle transport factor $\mathcal N_{\rm sol}$ as a geometric overlap correction to the leptonic RGEs. The main selected branch already establishes the load-bearing statement: the solar-angle drift is a transport effect of the anomaly-free kernel, not a hidden floating coordinate.

The anomaly-free branch defines a single fixed support map that passes the disclosed topological consistency checks. The reported pulls and $\chi^2$ tallies therefore follow directly from the branch-fixed construction.

The hierarchy statement is equally sharp: under the \emph{Minimal One-Copy Dictionary}, Normal Ordering is the minimal realization because it preserves the natural boundary-character order without duplicating support slots. If future data establish Inverted Ordering, the construction must pass to a non-minimal multi-copy extension. The present selected branch therefore singles out NO within the minimal anomaly-free branch.
...
\IfFileExists{gravity.tex}{\input{gravity}}{%
  \IfFileExists{papers/gravity.tex}{\input{papers/gravity}}{%
    \IfFileExists{../papers/gravity.tex}{\input{../papers/gravity}}{%
      \IfFileExists{pub/gravity.tex}{\input{pub/gravity}}{%
        \PackageWarning{tn}{Gravity appendix file not found; skipping appendix include}%
      }%
    }%
  }%
}
