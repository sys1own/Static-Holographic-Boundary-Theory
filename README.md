...
`map_rigidity_landscape.py` renders a benchmark-centered $5 \times 5 \times 5$ rigidity moat around $(26, 8, 312)$ and writes `results/rigidity_moat.png` plus `results/rigidity_moat.json`. The resulting 3D heatmap colors the scanned $(k_{\ell}, k_q, K)$ lattice by the framing-anomaly residue $\mathcal E = |\Delta_{\rm fr}|$, exposing the published branch as the singular stability well and every integer detuning as immediate decoherence.
